...
public class Uhh : MonoBehaviour {
    ...
    { 
        ...
        return Camera.main.ScreenToWorldPoint (hand.transform.position); 
    ...
    public static float AngleFromVector (Vector2 dir) { return Mathf.Atan2 (dir.y, dir.x) * Mathf.Rad2Deg - 90f; }
    public static Vector2 VectorFromAngle (float ang) { ang += 90f; ang *= Mathf.Deg2Rad; return new Vector2 (Mathf.Cos (ang), Mathf.Sin (ang)); }
    ...
    public static bool Offscreen (Vector2 pos, float radius = 0) {
        ...
        if (Mathf.Abs (radius) > 0.00001f) pos += (campos - pos).normalized * radius;
    ...
    public static void LoadLevel (string name) { SceneManager.LoadScene (name); }
